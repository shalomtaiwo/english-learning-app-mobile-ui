...
import { Button, Grid, Skeleton, Space, Text, Title } from '@mantine/core'
                        ...
                        skills?.map((items, index) => {
                            return (
                                <Grid.Col span={6} key={index}>
                                    <Skeleton visible={loading} height={170} mt={6} width="100%" radius="md">
                                        <MobileCards img
                                            direction={'column'}
                                            style={{
                                                padding: '15px 10px'
                                            ...
                                            <Space h={12} />
                                            <Title order={6}>{items?.title} </Title>
                                            <Space h={3} />
                                            <Text size={13} color='dimmed'>{items?.status}%</Text>
                                            <Space h={'sm'} />
                                            <Button
                                                color='orange'
                                                radius="xl"
                                                compact
                                                size="xs"
                                                variant={items.status === 100 ? 'filled' : 'outline'}
                                            >
                                                {items.status === 100 ? 'Completed' : 'Good work'}
                                            </Button>
                                        </MobileCards>
                                    </Skeleton>
                                </Grid.Col>
